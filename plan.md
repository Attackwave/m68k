...
## Umsetzungsstatus (Stand 2026-07-27)

**Erledigt: Phase 1 (alle 6), Phase 2 (alle 10), Phase 3 (alle 6), Phase 4 komplett (Punkte 1-9, siehe Detailstatus unten).**

Phase 4 (Branch `phase4-features`) umgesetzt: 4.2 (zentrales CPU-Gating), 4.3 (automatisierter Roundtrip-Test), 4.4 (Fuzzing-Setup), 4.5 (echte DC.X/DC.P-Konvertierung), 4.6 (Expression-Evaluatoren konsolidiert), 4.7 (m68k-floppy OFS/FFS + ADF-Schreiben), 4.9 (OPT-State-Tracking). Nach jedem Punkt verifiziert: `cargo test --all` (388 Tests in m68k-asm + 68 in m68k-floppy, alle Crates grün), `cargo clippy --all-targets -- -D warnings` sauber, `cargo fmt --all -- --check` sauber (auch für die separate `fuzz/`-Crate).

**Bemerkenswerte Zusatzfunde während Phase 4** (nicht ursprünglich geplant, beim Implementieren/Fuzzen gefunden und mitgefixt — siehe Commit-Historie auf `phase4-features` für Details):
- Beim Bau des Roundtrip-Tests (4.3): fehlender Size-Suffix beim Disassemblieren von TST/CLR/NOT/NEG/NEGX/ADDQ/SUBQ/EOR/ANDI/EORI/CMPI/ADDI/SUBI/ORI; MOVEC komplett unentdeckbar im Decoder (kein Opcode-Pattern).
- Beim Fuzzing (4.4/4.5): UTF-8-Char-Boundary-Panics in `tokens.rs` und in drei Operand-Parsern (`parse_parens_register*`); OOM in `amiga_hunk.rs::Reader::bytes` (Allokation vor Längenprüfung); Index-Out-of-Bounds bei `UNPK`/`PACK`/`CAS` ohne Operanden; `i32::MIN`-Abs-Panic in `format_signed`; Integer-Overflow bei Symbol-Adressen und bei 7 Branch-Displacement-Berechnungen im Disassembler; Hunk-Body-Länge vs. deklarierte Hunk-Größe konnten divergieren (Out-of-Range-Slice).
...
2. ✅ **Systematisches CPU-Gating**: neue zentrale Tabelle `m68k-core/src/cpu_gate.rs` (Mnemonic→`MinCpu`-Enum), an einer Stelle (`assembler.rs::encode_instruction_line`, vor jeder Verzweigung) aufgerufen — deckt Branch-Pfad, 3-Operanden-Sonderfälle und generischen Dispatch einheitlich ab. CLI-`--cpu`-Validierung (`validate_cpu_name`) in beiden Binaries ergänzt (unbekannte CPU-Strings wurden vorher stillschweigend als höchste Stufe interpretiert). Dabei mehrere zuvor komplett ungegatete Mnemonics gefunden: CAS2, PACK/UNPK, RTM, BKPT, RTD, MOVEC, FBcc/FDBcc/FTRAPcc/FNOP liefen auf `--cpu 68000` klaglos durch.
3. ✅ **Systematischer Roundtrip-Test**: `test_golden_vectors_full_roundtrip` in `crates/m68k-asm/tests/golden_assembler.rs`, alle 71 Golden-Encoder-Vektoren (68000/68020/68040) encode→decode→format→reassemble→re-encode, Byte-Vergleich gegen das Original. Deckte sofort zwei Bugs auf: fehlender Size-Suffix bei mehreren Disassembler-Mnemonics (TST/CLR/NOT/NEG/NEGX/ADDQ/SUBQ/EOR/ANDI/EORI/CMPI/ADDI/SUBI/ORI) und MOVEC komplett unentdeckbar im Decoder (kein Opcode-Pattern, jetzt ergänzt inkl. Kontrollregister-Reverse-Mapping).
4. ✅ **Fuzzing** (`cargo-fuzz`): 6 Targets unter `fuzz/` (`disassemble`, `amiga_hunk_parse`, `assembler_pipeline`, `floppy_adf`, `floppy_uae`, `floppy_ipf`), CI-Job `fuzz-smoke` mit 60s/Target. Fand beim Einrichten 8 echte Panics/Overflows/OOMs (siehe Zusatzfunde oben) — alle gefixt mit Regressionstests, danach mehrere Minuten/Millionen Iterationen pro Target ohne weitere Findings.
5. ✅ **DC.X/DC.P echte Konvertierung**: IEEE-754-80-bit-Extended aus `f64`-Dezimalliteral (inkl. Zero/Subnormal/Inf/NaN), echtes Packed-BCD für DC.P (Sign, BCD-Exponent, 17 BCD-Mantissendigits, Exponentenschreibweise). `$...`-Hex-Literale bleiben unverändert als Rohwert-Fallback. **FMOVE-k-factor-Syntax** (`FMOVE.P FPn,<ea>{#k}`/`{Dn}`) ebenfalls umgesetzt (Assembler + Disassembler, siehe PR #13) — Bit-Layout war in verfügbarer Doku ambig, empirisch gegen lokalen `vasm -m68881`-Build verifiziert (Unterscheidung statisch/dynamisch läuft über den Format-Code, nicht ein Bit im k-factor-Feld selbst). Dabei einen vorbestehenden Disassembler-Bug gefunden und gefixt (`FMOVE.P FPn,<ea>{...}` mit k≠0 dekodierte als "unknown FPU arithmetic opclass/opmode").
6. ✅ **Expression-Evaluatoren konsolidiert**: `m68k-core/src/expr.rs` (kein Parser, keine Symbol-Table-Anbindung, null externe Aufrufer) komplett entfernt. `directives.rs::parse_simple_expr` ist der alleinige, bereits vorher tatsächlich genutzte Evaluator. Rekursionslimit-Anfälligkeit geprüft und nicht vorhanden (feste Kette konkreter Funktionen, kein generisches `impl Fn`) — mit Regressionstest über 200-fach verschachtelten Ausdruck abgesichert.
7. ✅ **m68k-floppy-Ausbau**: neues `amigados`-Modul (read-only OFS/FFS: Root-Block/Hash-Table-Traversal, File-Header inkl. reverse-order Datenblock-Tabelle + Extension-Block-Chain, User-Directory rekursiv, OFS- und FFS-Datenblöcke), neues `adf_writer`-Modul (leere ADF-Images erzeugen, Bootblock-Checksumme berechnen/verifizieren/reparieren, `format_empty_ofs_disk`/`format_empty_ffs_disk`). CLI (`m68k-floppy`) um `--list`/`--extract NAME`/`--fix-bootblock` erweitert. End-to-End mit synthetischen Multi-Block-Dateien für OFS und FFS getestet.
...
9. ✅ **N2 — OPT-State-Tracking**: neues `OptState`-Struct (`asm.opt`), parst `OPT flag[n][+-]`-Syntax (vasm/Devpac-Stil), trackt einfache und nummerierte Flags. Malformte Einträge erzeugen jetzt eine Warnung statt lautlos verworfen zu werden.
